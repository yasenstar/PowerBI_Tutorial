# Part 2 - Chapter 6: Getting Data from Dynamic Sources

- [Part 2 - Chapter 6: Getting Data from Dynamic Sources](#part-2---chapter-6-getting-data-from-dynamic-sources)
  - [06.1 Getting Data from Microsoft-Based File Systems](#061-getting-data-from-microsoft-based-file-systems)
  - [06.2 Working with Relational Data Sources](#062-working-with-relational-data-sources)
  - [06.3 Importing Data from a Non-relational Data Source](#063-importing-data-from-a-non-relational-data-source)
  - [06.4 Importing JSON File Data into Power BI](#064-importing-json-file-data-into-power-bi)
  - [06.5 Importing Data from Online Sources](#065-importing-data-from-online-sources)
  - [06.6 Creating Data Source Combos](#066-creating-data-source-combos)
  - [06.7 Dealing with Modes for Dynamic Data](#067-dealing-with-modes-for-dynamic-data)
  - [06.8 Fixing Data Import Errors](#068-fixing-data-import-errors)

## 06.1 Getting Data from Microsoft-Based File Systems

- OneDrive
- SharePoint (Folder, List)

## 06.2 Working with Relational Data Sources

## 06.3 Importing Data from a Non-relational Data Source

## 06.4 Importing JSON File Data into Power BI

## 06.5 Importing Data from Online Sources

## 06.6 Creating Data Source Combos

- DAX: Data Analysis Expression language
- MDX: Multidimensional Data Expressions ([MDX Query Fundamentals](https://learn.microsoft.com/en-us/analysis-services/multidimensional-models/mdx/mdx-query-fundamentals-analysis-services?view=sql-analysis-services-2025))

## 06.7 Dealing with Modes for Dynamic Data

## 06.8 Fixing Data Import Errors
